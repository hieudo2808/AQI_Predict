...
def load_raw_data(aq_path='data/raw/open_meteo_aq.parquet', weather_path='data/raw/open_meteo_weather.parquet'):
    ...
    if os.path.exists(weather_path):
        ...
        logger.info(f"Đã tải và ghép nối dữ liệu AQ ({len(df_aq)} dòng) và Weather ({len(df_weather)} dòng).")
    else:
        logger.info(f"Không tìm thấy file dữ liệu Weather tại {weather_path}, chỉ sử dụng AQ.")
        df_raw = df_aq
...
def describe_data(df_raw):
    ...
    """
    logger.info(f'Dataset: {len(df_raw):,} bản ghi (giờ)')
    if 'date' in df_raw.columns and len(df_raw) > 0:
        logger.info(f'Thời gian: {df_raw["date"].min()} → {df_raw["date"].max()}')
    logger.info(f'Các cột: {df_raw.columns.tolist()}')
    ...
    if TARGET in df_raw.columns:
        logger.info(f'Thống kê mô tả {TARGET} (Target):')
        target_stats = df_raw[TARGET].describe()
        logger.info(f'\n{target_stats.round(2)}')
        logger.info(f'Số giờ {TARGET} > 35 µg/m³: {(df_raw[TARGET] > 35).sum()} / {len(df_raw)} giờ')
        logger.info(f'Số giờ {TARGET} > 75 µg/m³: {(df_raw[TARGET] > 75).sum()} / {len(df_raw)} giờ')

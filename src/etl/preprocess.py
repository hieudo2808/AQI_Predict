...
def handle_missing_values(df: pd.DataFrame) -> pd.DataFrame:
    """
    Xử lý missing values bằng cách gọi hàm preprocess(df).
    ...
    Returns:
        pd.DataFrame: DataFrame đã xử lý missing.
    """
    return preprocess(df)


def remove_outliers(df: pd.DataFrame, column: str = None, n_sigma: int = None) -> pd.DataFrame:
    """
    Loại bỏ outlier theo quy tắc n-sigma trên biến target.

    Parameters:
        df (pd.DataFrame): DataFrame cần xử lý.
        column (str, optional): Cột cần kiểm tra outlier. Mặc định là TARGET (pm2_5).
        n_sigma (int, optional): Số sigma. Mặc định là OUTLIER_N_SIGMA (3).

    Returns:
        pd.DataFrame: DataFrame đã loại bỏ outlier.
    """
    df = df.copy()
    col = column or TARGET
    sigma = n_sigma or OUTLIER_N_SIGMA

    if col not in df.columns:
        logger.warning(f"Cột {col} không tồn tại trong DataFrame. Bỏ qua loại bỏ outlier.")
        return df
    ...
    if pd.isna(mu) or pd.isna(std) or std == 0:
        logger.warning(f"Không thể tính toán mean/std cho cột {col}. Bỏ qua loại bỏ outlier.")
        return df

    df = df[(df[col] >= mu - sigma * std) & (df[col] <= mu + sigma * std)]

    removed = before - len(df)
    logger.info(f"✅ Loại bỏ outlier ({col}): {removed} bản ghi ngoài {sigma}σ ({before}→{len(df)})")
